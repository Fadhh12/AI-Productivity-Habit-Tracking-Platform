...
interface EmptyStateProps {
  icon: string;
  /** Show the mascot instead of the icon. */
  mascot?: MascotMood;
  title: string;
  children: ReactNode;
...
export function EmptyState({ icon, mascot, title, children, action }: EmptyStateProps) {
  const actionClass =
  ...
  return (
    <div className="flex animate-scale-in flex-col items-center rounded-2xl bg-surface-card px-space-lg py-space-xl text-center shadow-soft">
      {mascot ? (
        <Mascot mood={mascot} interactive className="force-light w-24" />
      ) : (
